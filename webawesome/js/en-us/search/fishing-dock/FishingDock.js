...
async function patchFishingDock($formFilters, $formValues, target, id, success, error) {
  ...
  patchFishingDockVals(id == null ? deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:'id:' + id}], vals, target, success, error);
}
...
async function deleteFishingDock(target, id, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target);
      var url = data['editPage'];
  ...
  fetch(
    '/en-us/api/fishing-dock/' + encodeURIComponent(id)
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportFishingDock($formValues, target, id, success, error) {
  ...
  if(id != null && vars.length > 0) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        jsWebsocketFishingDock(id, vars, $response);
