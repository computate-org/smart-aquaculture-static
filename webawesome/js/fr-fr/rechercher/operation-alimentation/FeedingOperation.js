...
async function websocketFeedingOperation(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketFeedingOperation', function (error, message) {
      ...
      $headerSpan.innerText = 'modify opération d'alimentations in ' + json.timeRemaining;
  ...
  if(entityShortId != null && vars.length > 0) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        if(vars.includes('pk'))
          inputPk = $response.querySelector('.FeedingOperation_Page_pk');
        if(vars.includes('created'))
          inputCreated = $response.querySelector('.FeedingOperation_Page_created');
        if(vars.includes('modified'))
          inputModified = $response.querySelector('.FeedingOperation_Page_modified');
        if(vars.includes('archived'))
          inputArchived = $response.querySelector('.FeedingOperation_Page_archived');
        if(vars.includes('name'))
          inputName = $response.querySelector('.FeedingOperation_Page_name');
        if(vars.includes('description'))
          inputDescription = $response.querySelector('.FeedingOperation_Page_description');
        if(vars.includes('location'))
          inputLocation = $response.querySelector('.FeedingOperation_Page_location');
        if(vars.includes('areaServed'))
          inputAreaServed = $response.querySelector('.FeedingOperation_Page_areaServed');
        if(vars.includes('id'))
          inputId = $response.querySelector('.FeedingOperation_Page_id');
        if(vars.includes('ngsildTenant'))
          inputNgsildTenant = $response.querySelector('.FeedingOperation_Page_ngsildTenant');
        if(vars.includes('ngsildPath'))
          inputNgsildPath = $response.querySelector('.FeedingOperation_Page_ngsildPath');
        if(vars.includes('ngsildContext'))
          inputNgsildContext = $response.querySelector('.FeedingOperation_Page_ngsildContext');
        if(vars.includes('ngsildData'))
          inputNgsildData = $response.querySelector('.FeedingOperation_Page_ngsildData');
        if(vars.includes('address'))
          inputAddress = $response.querySelector('.FeedingOperation_Page_address');
        if(vars.includes('alternateName'))
          inputAlternateName = $response.querySelector('.FeedingOperation_Page_alternateName');
        if(vars.includes('category'))
          inputCategory = $response.querySelector('.FeedingOperation_Page_category');
        if(vars.includes('dataProvider'))
          inputDataProvider = $response.querySelector('.FeedingOperation_Page_dataProvider');
        if(vars.includes('dateCreated'))
          inputDateCreated = $response.querySelector('.FeedingOperation_Page_dateCreated');
        if(vars.includes('dateModified'))
          inputDateModified = $response.querySelector('.FeedingOperation_Page_dateModified');
        if(vars.includes('endpoint'))
          inputEndpoint = $response.querySelector('.FeedingOperation_Page_endpoint');
        if(vars.includes('hasProvider'))
          inputHasProvider = $response.querySelector('.FeedingOperation_Page_hasProvider');
        if(vars.includes('owner'))
          inputOwner = $response.querySelector('.FeedingOperation_Page_owner');
        if(vars.includes('relatedSource'))
          inputRelatedSource = $response.querySelector('.FeedingOperation_Page_relatedSource');
        if(vars.includes('seeAlso'))
          inputSeeAlso = $response.querySelector('.FeedingOperation_Page_seeAlso');
        if(vars.includes('source'))
          inputSource = $response.querySelector('.FeedingOperation_Page_source');
        if(vars.includes('version'))
          inputVersion = $response.querySelector('.FeedingOperation_Page_version');
        if(vars.includes('classCanonicalName'))
          inputClassCanonicalName = $response.querySelector('.FeedingOperation_Page_classCanonicalName');
        if(vars.includes('classSimpleName'))
          inputClassSimpleName = $response.querySelector('.FeedingOperation_Page_classSimpleName');
        if(vars.includes('classCanonicalNames'))
          inputClassCanonicalNames = $response.querySelector('.FeedingOperation_Page_classCanonicalNames');
        if(vars.includes('sessionId'))
          inputSessionId = $response.querySelector('.FeedingOperation_Page_sessionId');
        if(vars.includes('userKey'))
          inputUserKey = $response.querySelector('.FeedingOperation_Page_userKey');
        if(vars.includes('saves'))
          inputSaves = $response.querySelector('.FeedingOperation_Page_saves');
        if(vars.includes('objectTitle'))
          inputObjectTitle = $response.querySelector('.FeedingOperation_Page_objectTitle');
        if(vars.includes('displayPage'))
          inputDisplayPage = $response.querySelector('.FeedingOperation_Page_displayPage');
        if(vars.includes('displayPageFrFR'))
          inputDisplayPageFrFR = $response.querySelector('.FeedingOperation_Page_displayPageFrFR');
        if(vars.includes('editPage'))
          inputEditPage = $response.querySelector('.FeedingOperation_Page_editPage');
        if(vars.includes('editPageFrFR'))
          inputEditPageFrFR = $response.querySelector('.FeedingOperation_Page_editPageFrFR');
        if(vars.includes('userPage'))
          inputUserPage = $response.querySelector('.FeedingOperation_Page_userPage');
        if(vars.includes('userPageFrFR'))
          inputUserPageFrFR = $response.querySelector('.FeedingOperation_Page_userPageFrFR');
        if(vars.includes('download'))
          inputDownload = $response.querySelector('.FeedingOperation_Page_download');
        if(vars.includes('downloadFrFR'))
          inputDownloadFrFR = $response.querySelector('.FeedingOperation_Page_downloadFrFR');
        if(vars.includes('objectSuggest'))
          inputObjectSuggest = $response.querySelector('.FeedingOperation_Page_objectSuggest');
        if(vars.includes('objectText'))
          inputObjectText = $response.querySelector('.FeedingOperation_Page_objectText');
        if(vars.includes('solrId'))
          inputSolrId = $response.querySelector('.FeedingOperation_Page_solrId');
        if(vars.includes('areaServedColors'))
          inputAreaServedColors = $response.querySelector('.FeedingOperation_Page_areaServedColors');
        if(vars.includes('areaServedTitles'))
          inputAreaServedTitles = $response.querySelector('.FeedingOperation_Page_areaServedTitles');
        if(vars.includes('areaServedLinks'))
          inputAreaServedLinks = $response.querySelector('.FeedingOperation_Page_areaServedLinks');
        if(vars.includes('entityShortId'))
          inputEntityShortId = $response.querySelector('.FeedingOperation_Page_entityShortId');
        ...
        if(inputPk) {
          document.querySelectorAll('.FeedingOperation_Page_pk').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPk.getAttribute('value');
            else
              item.textContent = inputPk.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_pk'));
        }

        if(inputCreated) {
          document.querySelectorAll('.FeedingOperation_Page_created').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputCreated.getAttribute('value');
            else
              item.textContent = inputCreated.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_created'));
        }

        if(inputModified) {
          document.querySelectorAll('.FeedingOperation_Page_modified').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputModified.getAttribute('value');
            else
              item.textContent = inputModified.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_modified'));
        }

        if(inputArchived) {
          document.querySelectorAll('.FeedingOperation_Page_archived').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputArchived.getAttribute('value');
            else
              item.textContent = inputArchived.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_archived'));
        }

        if(inputName) {
          document.querySelectorAll('.FeedingOperation_Page_name').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputName.getAttribute('value');
            else
              item.textContent = inputName.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_name'));
        }

        if(inputDescription) {
          document.querySelectorAll('.FeedingOperation_Page_description').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDescription.getAttribute('value');
            else
              item.textContent = inputDescription.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_description'));
        }

        if(inputLocation) {
          document.querySelectorAll('.FeedingOperation_Page_location').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputLocation.getAttribute('value');
            else
              item.textContent = inputLocation.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_location'));
        }

        if(inputAreaServed) {
          document.querySelectorAll('.FeedingOperation_Page_areaServed').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAreaServed.getAttribute('value');
            else
              item.textContent = inputAreaServed.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_areaServed'));
        }

        if(inputId) {
          document.querySelectorAll('.FeedingOperation_Page_id').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputId.getAttribute('value');
            else
              item.textContent = inputId.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_id'));
        }

        if(inputNgsildTenant) {
          document.querySelectorAll('.FeedingOperation_Page_ngsildTenant').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildTenant.getAttribute('value');
            else
              item.textContent = inputNgsildTenant.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_ngsildTenant'));
        }

        if(inputNgsildPath) {
          document.querySelectorAll('.FeedingOperation_Page_ngsildPath').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildPath.getAttribute('value');
            else
              item.textContent = inputNgsildPath.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_ngsildPath'));
        }

        if(inputNgsildContext) {
          document.querySelectorAll('.FeedingOperation_Page_ngsildContext').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildContext.getAttribute('value');
            else
              item.textContent = inputNgsildContext.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_ngsildContext'));
        }

        if(inputNgsildData) {
          document.querySelectorAll('.FeedingOperation_Page_ngsildData').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildData.getAttribute('value');
            else
              item.textContent = inputNgsildData.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_ngsildData'));
        }

        if(inputAddress) {
          document.querySelectorAll('.FeedingOperation_Page_address').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAddress.getAttribute('value');
            else
              item.textContent = inputAddress.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_address'));
        }

        if(inputAlternateName) {
          document.querySelectorAll('.FeedingOperation_Page_alternateName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAlternateName.getAttribute('value');
            else
              item.textContent = inputAlternateName.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_alternateName'));
        }

        if(inputCategory) {
          document.querySelectorAll('.FeedingOperation_Page_category').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputCategory.getAttribute('value');
            else
              item.textContent = inputCategory.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_category'));
        }

        if(inputDataProvider) {
          document.querySelectorAll('.FeedingOperation_Page_dataProvider').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDataProvider.getAttribute('value');
            else
              item.textContent = inputDataProvider.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_dataProvider'));
        }

        if(inputDateCreated) {
          document.querySelectorAll('.FeedingOperation_Page_dateCreated').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDateCreated.getAttribute('value');
            else
              item.textContent = inputDateCreated.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_dateCreated'));
        }

        if(inputDateModified) {
          document.querySelectorAll('.FeedingOperation_Page_dateModified').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDateModified.getAttribute('value');
            else
              item.textContent = inputDateModified.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_dateModified'));
        }

        if(inputEndpoint) {
          document.querySelectorAll('.FeedingOperation_Page_endpoint').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEndpoint.getAttribute('value');
            else
              item.textContent = inputEndpoint.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_endpoint'));
        }

        if(inputHasProvider) {
          document.querySelectorAll('.FeedingOperation_Page_hasProvider').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputHasProvider.getAttribute('value');
            else
              item.textContent = inputHasProvider.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_hasProvider'));
        }

        if(inputOwner) {
          document.querySelectorAll('.FeedingOperation_Page_owner').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputOwner.getAttribute('value');
            else
              item.textContent = inputOwner.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_owner'));
        }

        if(inputRelatedSource) {
          document.querySelectorAll('.FeedingOperation_Page_relatedSource').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputRelatedSource.getAttribute('value');
            else
              item.textContent = inputRelatedSource.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_relatedSource'));
        }

        if(inputSeeAlso) {
          document.querySelectorAll('.FeedingOperation_Page_seeAlso').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSeeAlso.getAttribute('value');
            else
              item.textContent = inputSeeAlso.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_seeAlso'));
        }

        if(inputSource) {
          document.querySelectorAll('.FeedingOperation_Page_source').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSource.getAttribute('value');
            else
              item.textContent = inputSource.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_source'));
        }

        if(inputVersion) {
          document.querySelectorAll('.FeedingOperation_Page_version').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputVersion.getAttribute('value');
            else
              item.textContent = inputVersion.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_version'));
        }

        if(inputClassCanonicalName) {
          document.querySelectorAll('.FeedingOperation_Page_classCanonicalName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalName.getAttribute('value');
            else
              item.textContent = inputClassCanonicalName.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_classCanonicalName'));
        }

        if(inputClassSimpleName) {
          document.querySelectorAll('.FeedingOperation_Page_classSimpleName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassSimpleName.getAttribute('value');
            else
              item.textContent = inputClassSimpleName.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_classSimpleName'));
        }

        if(inputClassCanonicalNames) {
          document.querySelectorAll('.FeedingOperation_Page_classCanonicalNames').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalNames.getAttribute('value');
            else
              item.textContent = inputClassCanonicalNames.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_classCanonicalNames'));
        }

        if(inputSessionId) {
          document.querySelectorAll('.FeedingOperation_Page_sessionId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSessionId.getAttribute('value');
            else
              item.textContent = inputSessionId.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_sessionId'));
        }

        if(inputUserKey) {
          document.querySelectorAll('.FeedingOperation_Page_userKey').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserKey.getAttribute('value');
            else
              item.textContent = inputUserKey.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_userKey'));
        }

        if(inputSaves) {
          document.querySelectorAll('.FeedingOperation_Page_saves').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSaves.getAttribute('value');
            else
              item.textContent = inputSaves.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_saves'));
        }

        if(inputObjectTitle) {
          document.querySelectorAll('.FeedingOperation_Page_objectTitle').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectTitle.getAttribute('value');
            else
              item.textContent = inputObjectTitle.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_objectTitle'));
        }

        if(inputDisplayPage) {
          document.querySelectorAll('.FeedingOperation_Page_displayPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDisplayPage.getAttribute('value');
            else
              item.textContent = inputDisplayPage.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_displayPage'));
        }

        if(inputDisplayPageFrFR) {
          document.querySelectorAll('.FeedingOperation_Page_displayPageFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDisplayPageFrFR.getAttribute('value');
            else
              item.textContent = inputDisplayPageFrFR.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_displayPageFrFR'));
        }

        if(inputEditPage) {
          document.querySelectorAll('.FeedingOperation_Page_editPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEditPage.getAttribute('value');
            else
              item.textContent = inputEditPage.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_editPage'));
        }

        if(inputEditPageFrFR) {
          document.querySelectorAll('.FeedingOperation_Page_editPageFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEditPageFrFR.getAttribute('value');
            else
              item.textContent = inputEditPageFrFR.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_editPageFrFR'));
        }

        if(inputUserPage) {
          document.querySelectorAll('.FeedingOperation_Page_userPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserPage.getAttribute('value');
            else
              item.textContent = inputUserPage.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_userPage'));
        }

        if(inputUserPageFrFR) {
          document.querySelectorAll('.FeedingOperation_Page_userPageFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserPageFrFR.getAttribute('value');
            else
              item.textContent = inputUserPageFrFR.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_userPageFrFR'));
        }

        if(inputDownload) {
          document.querySelectorAll('.FeedingOperation_Page_download').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDownload.getAttribute('value');
            else
              item.textContent = inputDownload.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_download'));
        }

        if(inputDownloadFrFR) {
          document.querySelectorAll('.FeedingOperation_Page_downloadFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDownloadFrFR.getAttribute('value');
            else
              item.textContent = inputDownloadFrFR.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_downloadFrFR'));
        }

        if(inputObjectSuggest) {
          document.querySelectorAll('.FeedingOperation_Page_objectSuggest').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectSuggest.getAttribute('value');
            else
              item.textContent = inputObjectSuggest.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_objectSuggest'));
        }

        if(inputObjectText) {
          document.querySelectorAll('.FeedingOperation_Page_objectText').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectText.getAttribute('value');
            else
              item.textContent = inputObjectText.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_objectText'));
        }

        if(inputSolrId) {
          document.querySelectorAll('.FeedingOperation_Page_solrId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSolrId.getAttribute('value');
            else
              item.textContent = inputSolrId.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_solrId'));
        }

        if(inputAreaServedColors) {
          document.querySelectorAll('.FeedingOperation_Page_areaServedColors').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAreaServedColors.getAttribute('value');
            else
              item.textContent = inputAreaServedColors.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_areaServedColors'));
        }

        if(inputAreaServedTitles) {
          document.querySelectorAll('.FeedingOperation_Page_areaServedTitles').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAreaServedTitles.getAttribute('value');
            else
              item.textContent = inputAreaServedTitles.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_areaServedTitles'));
        }

        if(inputAreaServedLinks) {
          document.querySelectorAll('.FeedingOperation_Page_areaServedLinks').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAreaServedLinks.getAttribute('value');
            else
              item.textContent = inputAreaServedLinks.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_areaServedLinks'));
        }

        if(inputEntityShortId) {
          document.querySelectorAll('.FeedingOperation_Page_entityShortId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEntityShortId.getAttribute('value');
            else
              item.textContent = inputEntityShortId.textContent;
          });
          addGlow(document.querySelector('.FeedingOperation_Page_entityShortId'));
        }
  ...
  if(r) {
    ...
    if(json['facetCounts']) {
      ...
      if(facetCounts['facetPivot'] && facetCounts['facetRanges']) {
        ...
        var layout = {};
        if(range) {
          layout['title'] = 'opération d'alimentations';
          layout['xaxis'] = {
        ...
        if(event.layerType == 'marker') {
          contextmenuItems.push({
            ...
            , callback: function(event2) {
              patchFeedingOperationLocation(event.layer, { coordinates: [event.layer.getLatLng()['lng'], event.layer.getLatLng()['lat']], type: "Point" });
            }
          });
        }
        if(event.layerType == 'polygon') {
          contextmenuItems.push({
            ...
            , callback: function(event2) {
              ...
              patchFeedingOperationArea(event.layer, { coordinates: latLngs, type: "Polygon" });
            }
          });
        }
        if(event.layerType == 'polyline') {
          contextmenuItems.push({
            ...
            , callback: function(event2) {
              ...
              patchFeedingOperationArea(event.layer, { coordinates: latLngs, type: "LineString" });
            }
          });
        }
...
function patchFeedingOperationLocation(target, location) {
...
function patchFeedingOperationArea(target, areaServed) {
  patchFeedingOperationVal([{ name: 'softCommit', value: 'true' }, { name: 'fq', value: 'entityShortId:' + result.entityShortId }]
