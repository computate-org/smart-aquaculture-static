...
async function websocketFishFarm(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketFishFarm', function (error, message) {
      ...
      $headerSpan.innerText = 'modify ferme piscicoles in ' + json.timeRemaining;
  ...
  if(entityShortId != null && vars.length > 0) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        if(vars.includes('pk'))
          inputPk = $response.querySelector('.FishFarm_Page_pk');
        if(vars.includes('created'))
          inputCreated = $response.querySelector('.FishFarm_Page_created');
        if(vars.includes('modified'))
          inputModified = $response.querySelector('.FishFarm_Page_modified');
        if(vars.includes('archived'))
          inputArchived = $response.querySelector('.FishFarm_Page_archived');
        if(vars.includes('name'))
          inputName = $response.querySelector('.FishFarm_Page_name');
        if(vars.includes('description'))
          inputDescription = $response.querySelector('.FishFarm_Page_description');
        if(vars.includes('areaServed'))
          inputAreaServed = $response.querySelector('.FishFarm_Page_areaServed');
        if(vars.includes('location'))
          inputLocation = $response.querySelector('.FishFarm_Page_location');
        if(vars.includes('color'))
          inputColor = $response.querySelector('.FishFarm_Page_color');
        if(vars.includes('id'))
          inputId = $response.querySelector('.FishFarm_Page_id');
        if(vars.includes('ngsildTenant'))
          inputNgsildTenant = $response.querySelector('.FishFarm_Page_ngsildTenant');
        if(vars.includes('ngsildPath'))
          inputNgsildPath = $response.querySelector('.FishFarm_Page_ngsildPath');
        if(vars.includes('ngsildContext'))
          inputNgsildContext = $response.querySelector('.FishFarm_Page_ngsildContext');
        if(vars.includes('ngsildData'))
          inputNgsildData = $response.querySelector('.FishFarm_Page_ngsildData');
        if(vars.includes('address'))
          inputAddress = $response.querySelector('.FishFarm_Page_address');
        if(vars.includes('classCanonicalName'))
          inputClassCanonicalName = $response.querySelector('.FishFarm_Page_classCanonicalName');
        if(vars.includes('classSimpleName'))
          inputClassSimpleName = $response.querySelector('.FishFarm_Page_classSimpleName');
        if(vars.includes('classCanonicalNames'))
          inputClassCanonicalNames = $response.querySelector('.FishFarm_Page_classCanonicalNames');
        if(vars.includes('sessionId'))
          inputSessionId = $response.querySelector('.FishFarm_Page_sessionId');
        if(vars.includes('userKey'))
          inputUserKey = $response.querySelector('.FishFarm_Page_userKey');
        if(vars.includes('saves'))
          inputSaves = $response.querySelector('.FishFarm_Page_saves');
        if(vars.includes('objectTitle'))
          inputObjectTitle = $response.querySelector('.FishFarm_Page_objectTitle');
        if(vars.includes('displayPage'))
          inputDisplayPage = $response.querySelector('.FishFarm_Page_displayPage');
        if(vars.includes('displayPageFrFR'))
          inputDisplayPageFrFR = $response.querySelector('.FishFarm_Page_displayPageFrFR');
        if(vars.includes('editPage'))
          inputEditPage = $response.querySelector('.FishFarm_Page_editPage');
        if(vars.includes('editPageFrFR'))
          inputEditPageFrFR = $response.querySelector('.FishFarm_Page_editPageFrFR');
        if(vars.includes('userPage'))
          inputUserPage = $response.querySelector('.FishFarm_Page_userPage');
        if(vars.includes('userPageFrFR'))
          inputUserPageFrFR = $response.querySelector('.FishFarm_Page_userPageFrFR');
        if(vars.includes('download'))
          inputDownload = $response.querySelector('.FishFarm_Page_download');
        if(vars.includes('downloadFrFR'))
          inputDownloadFrFR = $response.querySelector('.FishFarm_Page_downloadFrFR');
        if(vars.includes('objectSuggest'))
          inputObjectSuggest = $response.querySelector('.FishFarm_Page_objectSuggest');
        if(vars.includes('objectText'))
          inputObjectText = $response.querySelector('.FishFarm_Page_objectText');
        if(vars.includes('solrId'))
          inputSolrId = $response.querySelector('.FishFarm_Page_solrId');
        if(vars.includes('entityShortId'))
          inputEntityShortId = $response.querySelector('.FishFarm_Page_entityShortId');
        if(vars.includes('areaServedColors'))
          inputAreaServedColors = $response.querySelector('.FishFarm_Page_areaServedColors');
        if(vars.includes('areaServedTitles'))
          inputAreaServedTitles = $response.querySelector('.FishFarm_Page_areaServedTitles');
        ...
        if(inputPk) {
          document.querySelectorAll('.FishFarm_Page_pk').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPk.getAttribute('value');
            else
              item.textContent = inputPk.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_pk'));
        }

        if(inputCreated) {
          document.querySelectorAll('.FishFarm_Page_created').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputCreated.getAttribute('value');
            else
              item.textContent = inputCreated.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_created'));
        }

        if(inputModified) {
          document.querySelectorAll('.FishFarm_Page_modified').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputModified.getAttribute('value');
            else
              item.textContent = inputModified.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_modified'));
        }

        if(inputArchived) {
          document.querySelectorAll('.FishFarm_Page_archived').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputArchived.getAttribute('value');
            else
              item.textContent = inputArchived.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_archived'));
        }

        if(inputName) {
          document.querySelectorAll('.FishFarm_Page_name').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputName.getAttribute('value');
            else
              item.textContent = inputName.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_name'));
        }

        if(inputDescription) {
          document.querySelectorAll('.FishFarm_Page_description').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDescription.getAttribute('value');
            else
              item.textContent = inputDescription.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_description'));
        }

        if(inputAreaServed) {
          document.querySelectorAll('.FishFarm_Page_areaServed').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAreaServed.getAttribute('value');
            else
              item.textContent = inputAreaServed.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_areaServed'));
        }

        if(inputLocation) {
          document.querySelectorAll('.FishFarm_Page_location').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputLocation.getAttribute('value');
            else
              item.textContent = inputLocation.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_location'));
        }

        if(inputColor) {
          document.querySelectorAll('.FishFarm_Page_color').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputColor.getAttribute('value');
            else
              item.textContent = inputColor.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_color'));
        }

        if(inputId) {
          document.querySelectorAll('.FishFarm_Page_id').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputId.getAttribute('value');
            else
              item.textContent = inputId.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_id'));
        }

        if(inputNgsildTenant) {
          document.querySelectorAll('.FishFarm_Page_ngsildTenant').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildTenant.getAttribute('value');
            else
              item.textContent = inputNgsildTenant.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_ngsildTenant'));
        }

        if(inputNgsildPath) {
          document.querySelectorAll('.FishFarm_Page_ngsildPath').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildPath.getAttribute('value');
            else
              item.textContent = inputNgsildPath.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_ngsildPath'));
        }

        if(inputNgsildContext) {
          document.querySelectorAll('.FishFarm_Page_ngsildContext').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildContext.getAttribute('value');
            else
              item.textContent = inputNgsildContext.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_ngsildContext'));
        }

        if(inputNgsildData) {
          document.querySelectorAll('.FishFarm_Page_ngsildData').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildData.getAttribute('value');
            else
              item.textContent = inputNgsildData.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_ngsildData'));
        }

        if(inputAddress) {
          document.querySelectorAll('.FishFarm_Page_address').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAddress.getAttribute('value');
            else
              item.textContent = inputAddress.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_address'));
        }

        if(inputClassCanonicalName) {
          document.querySelectorAll('.FishFarm_Page_classCanonicalName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalName.getAttribute('value');
            else
              item.textContent = inputClassCanonicalName.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_classCanonicalName'));
        }

        if(inputClassSimpleName) {
          document.querySelectorAll('.FishFarm_Page_classSimpleName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassSimpleName.getAttribute('value');
            else
              item.textContent = inputClassSimpleName.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_classSimpleName'));
        }

        if(inputClassCanonicalNames) {
          document.querySelectorAll('.FishFarm_Page_classCanonicalNames').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalNames.getAttribute('value');
            else
              item.textContent = inputClassCanonicalNames.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_classCanonicalNames'));
        }

        if(inputSessionId) {
          document.querySelectorAll('.FishFarm_Page_sessionId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSessionId.getAttribute('value');
            else
              item.textContent = inputSessionId.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_sessionId'));
        }

        if(inputUserKey) {
          document.querySelectorAll('.FishFarm_Page_userKey').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserKey.getAttribute('value');
            else
              item.textContent = inputUserKey.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_userKey'));
        }

        if(inputSaves) {
          document.querySelectorAll('.FishFarm_Page_saves').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSaves.getAttribute('value');
            else
              item.textContent = inputSaves.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_saves'));
        }

        if(inputObjectTitle) {
          document.querySelectorAll('.FishFarm_Page_objectTitle').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectTitle.getAttribute('value');
            else
              item.textContent = inputObjectTitle.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_objectTitle'));
        }

        if(inputDisplayPage) {
          document.querySelectorAll('.FishFarm_Page_displayPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDisplayPage.getAttribute('value');
            else
              item.textContent = inputDisplayPage.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_displayPage'));
        }

        if(inputDisplayPageFrFR) {
          document.querySelectorAll('.FishFarm_Page_displayPageFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDisplayPageFrFR.getAttribute('value');
            else
              item.textContent = inputDisplayPageFrFR.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_displayPageFrFR'));
        }

        if(inputEditPage) {
          document.querySelectorAll('.FishFarm_Page_editPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEditPage.getAttribute('value');
            else
              item.textContent = inputEditPage.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_editPage'));
        }

        if(inputEditPageFrFR) {
          document.querySelectorAll('.FishFarm_Page_editPageFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEditPageFrFR.getAttribute('value');
            else
              item.textContent = inputEditPageFrFR.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_editPageFrFR'));
        }

        if(inputUserPage) {
          document.querySelectorAll('.FishFarm_Page_userPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserPage.getAttribute('value');
            else
              item.textContent = inputUserPage.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_userPage'));
        }

        if(inputUserPageFrFR) {
          document.querySelectorAll('.FishFarm_Page_userPageFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserPageFrFR.getAttribute('value');
            else
              item.textContent = inputUserPageFrFR.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_userPageFrFR'));
        }

        if(inputDownload) {
          document.querySelectorAll('.FishFarm_Page_download').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDownload.getAttribute('value');
            else
              item.textContent = inputDownload.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_download'));
        }

        if(inputDownloadFrFR) {
          document.querySelectorAll('.FishFarm_Page_downloadFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDownloadFrFR.getAttribute('value');
            else
              item.textContent = inputDownloadFrFR.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_downloadFrFR'));
        }

        if(inputObjectSuggest) {
          document.querySelectorAll('.FishFarm_Page_objectSuggest').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectSuggest.getAttribute('value');
            else
              item.textContent = inputObjectSuggest.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_objectSuggest'));
        }

        if(inputObjectText) {
          document.querySelectorAll('.FishFarm_Page_objectText').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectText.getAttribute('value');
            else
              item.textContent = inputObjectText.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_objectText'));
        }

        if(inputSolrId) {
          document.querySelectorAll('.FishFarm_Page_solrId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSolrId.getAttribute('value');
            else
              item.textContent = inputSolrId.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_solrId'));
        }

        if(inputEntityShortId) {
          document.querySelectorAll('.FishFarm_Page_entityShortId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEntityShortId.getAttribute('value');
            else
              item.textContent = inputEntityShortId.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_entityShortId'));
        }

        if(inputAreaServedColors) {
          document.querySelectorAll('.FishFarm_Page_areaServedColors').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAreaServedColors.getAttribute('value');
            else
              item.textContent = inputAreaServedColors.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_areaServedColors'));
        }

        if(inputAreaServedTitles) {
          document.querySelectorAll('.FishFarm_Page_areaServedTitles').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAreaServedTitles.getAttribute('value');
            else
              item.textContent = inputAreaServedTitles.textContent;
          });
          addGlow(document.querySelector('.FishFarm_Page_areaServedTitles'));
        }
        ...
        if(event.layerType == 'marker') {
          contextmenuItems.push({
            ...
            , callback: function(event2) {
              patchFishFarmLocation(event.layer, { coordinates: [event.layer.getLatLng()['lng'], event.layer.getLatLng()['lat']], type: "Point" });
            }
          });
        }
        if(event.layerType == 'polygon') {
          contextmenuItems.push({
            ...
            , callback: function(event2) {
              ...
              patchFishFarmArea(event.layer, { coordinates: latLngs, type: "Polygon" });
            }
          });
        }
        if(event.layerType == 'polyline') {
          contextmenuItems.push({
            ...
            , callback: function(event2) {
              ...
              patchFishFarmArea(event.layer, { coordinates: latLngs, type: "LineString" });
            }
          });
        }
...
function patchFishFarmLocation(target, location) {
...
function patchFishFarmArea(target, areaServed) {
  patchFishFarmVal([{ name: 'softCommit', value: 'true' }, { name: 'fq', value: 'entityShortId:' + result.entityShortId }]
