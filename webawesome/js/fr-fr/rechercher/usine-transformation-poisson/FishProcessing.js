...
async function websocketFishProcessing(success) {
  window.eventBus.onopen = function () {

    window.eventBus.registerHandler('websocketFishProcessing', function (error, message) {
      ...
      iTemplate.innerHTML = '<i class="fa-duotone fa-regular fa-industry-windows"></i>';
      var $i = iTemplate.content;
      var $headerSpan = document.createElement('span');
      $headerSpan.setAttribute('class', '');
      $headerSpan.innerText = 'modify usine de transformation du poissons in ' + json.timeRemaining;
...
async function websocketFishProcessingInner(apiRequest) {
  ...
  if(entityShortId != null && vars.length > 0) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        if(vars.includes('pk'))
          inputPk = $response.querySelector('.FishProcessing_Page_pk');
        if(vars.includes('created'))
          inputCreated = $response.querySelector('.FishProcessing_Page_created');
        if(vars.includes('modified'))
          inputModified = $response.querySelector('.FishProcessing_Page_modified');
        if(vars.includes('archived'))
          inputArchived = $response.querySelector('.FishProcessing_Page_archived');
        if(vars.includes('name'))
          inputName = $response.querySelector('.FishProcessing_Page_name');
        if(vars.includes('description'))
          inputDescription = $response.querySelector('.FishProcessing_Page_description');
        if(vars.includes('areaServed'))
          inputAreaServed = $response.querySelector('.FishProcessing_Page_areaServed');
        if(vars.includes('location'))
          inputLocation = $response.querySelector('.FishProcessing_Page_location');
        if(vars.includes('color'))
          inputColor = $response.querySelector('.FishProcessing_Page_color');
        if(vars.includes('id'))
          inputId = $response.querySelector('.FishProcessing_Page_id');
        if(vars.includes('ngsildTenant'))
          inputNgsildTenant = $response.querySelector('.FishProcessing_Page_ngsildTenant');
        if(vars.includes('ngsildPath'))
          inputNgsildPath = $response.querySelector('.FishProcessing_Page_ngsildPath');
        if(vars.includes('ngsildContext'))
          inputNgsildContext = $response.querySelector('.FishProcessing_Page_ngsildContext');
        if(vars.includes('ngsildData'))
          inputNgsildData = $response.querySelector('.FishProcessing_Page_ngsildData');
        if(vars.includes('address'))
          inputAddress = $response.querySelector('.FishProcessing_Page_address');
        if(vars.includes('classCanonicalName'))
          inputClassCanonicalName = $response.querySelector('.FishProcessing_Page_classCanonicalName');
        if(vars.includes('classSimpleName'))
          inputClassSimpleName = $response.querySelector('.FishProcessing_Page_classSimpleName');
        if(vars.includes('classCanonicalNames'))
          inputClassCanonicalNames = $response.querySelector('.FishProcessing_Page_classCanonicalNames');
        if(vars.includes('sessionId'))
          inputSessionId = $response.querySelector('.FishProcessing_Page_sessionId');
        if(vars.includes('userKey'))
          inputUserKey = $response.querySelector('.FishProcessing_Page_userKey');
        if(vars.includes('saves'))
          inputSaves = $response.querySelector('.FishProcessing_Page_saves');
        if(vars.includes('objectTitle'))
          inputObjectTitle = $response.querySelector('.FishProcessing_Page_objectTitle');
        if(vars.includes('displayPage'))
          inputDisplayPage = $response.querySelector('.FishProcessing_Page_displayPage');
        if(vars.includes('displayPageFrFR'))
          inputDisplayPageFrFR = $response.querySelector('.FishProcessing_Page_displayPageFrFR');
        if(vars.includes('editPage'))
          inputEditPage = $response.querySelector('.FishProcessing_Page_editPage');
        if(vars.includes('editPageFrFR'))
          inputEditPageFrFR = $response.querySelector('.FishProcessing_Page_editPageFrFR');
        if(vars.includes('userPage'))
          inputUserPage = $response.querySelector('.FishProcessing_Page_userPage');
        if(vars.includes('userPageFrFR'))
          inputUserPageFrFR = $response.querySelector('.FishProcessing_Page_userPageFrFR');
        if(vars.includes('download'))
          inputDownload = $response.querySelector('.FishProcessing_Page_download');
        if(vars.includes('downloadFrFR'))
          inputDownloadFrFR = $response.querySelector('.FishProcessing_Page_downloadFrFR');
        if(vars.includes('objectSuggest'))
          inputObjectSuggest = $response.querySelector('.FishProcessing_Page_objectSuggest');
        if(vars.includes('objectText'))
          inputObjectText = $response.querySelector('.FishProcessing_Page_objectText');
        if(vars.includes('solrId'))
          inputSolrId = $response.querySelector('.FishProcessing_Page_solrId');
        if(vars.includes('entityShortId'))
          inputEntityShortId = $response.querySelector('.FishProcessing_Page_entityShortId');
        if(vars.includes('areaServedColors'))
          inputAreaServedColors = $response.querySelector('.FishProcessing_Page_areaServedColors');

        jsWebsocketFishProcessing(entityShortId, vars, $response);
        window.result = JSON.parse($response.querySelector('.pageForm .result')?.value);
        window.listFishProcessing = JSON.parse($response.querySelector('.pageForm .listFishProcessing')?.value);


        if(inputPk) {
          document.querySelectorAll('.FishProcessing_Page_pk').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPk.getAttribute('value');
            else
              item.textContent = inputPk.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_pk'));
        }

        if(inputCreated) {
          document.querySelectorAll('.FishProcessing_Page_created').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputCreated.getAttribute('value');
            else
              item.textContent = inputCreated.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_created'));
        }

        if(inputModified) {
          document.querySelectorAll('.FishProcessing_Page_modified').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputModified.getAttribute('value');
            else
              item.textContent = inputModified.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_modified'));
        }

        if(inputArchived) {
          document.querySelectorAll('.FishProcessing_Page_archived').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputArchived.getAttribute('value');
            else
              item.textContent = inputArchived.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_archived'));
        }

        if(inputName) {
          document.querySelectorAll('.FishProcessing_Page_name').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputName.getAttribute('value');
            else
              item.textContent = inputName.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_name'));
        }

        if(inputDescription) {
          document.querySelectorAll('.FishProcessing_Page_description').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDescription.getAttribute('value');
            else
              item.textContent = inputDescription.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_description'));
        }

        if(inputAreaServed) {
          document.querySelectorAll('.FishProcessing_Page_areaServed').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAreaServed.getAttribute('value');
            else
              item.textContent = inputAreaServed.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_areaServed'));
        }

        if(inputLocation) {
          document.querySelectorAll('.FishProcessing_Page_location').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputLocation.getAttribute('value');
            else
              item.textContent = inputLocation.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_location'));
        }

        if(inputColor) {
          document.querySelectorAll('.FishProcessing_Page_color').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputColor.getAttribute('value');
            else
              item.textContent = inputColor.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_color'));
        }

        if(inputId) {
          document.querySelectorAll('.FishProcessing_Page_id').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputId.getAttribute('value');
            else
              item.textContent = inputId.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_id'));
        }

        if(inputNgsildTenant) {
          document.querySelectorAll('.FishProcessing_Page_ngsildTenant').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildTenant.getAttribute('value');
            else
              item.textContent = inputNgsildTenant.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_ngsildTenant'));
        }

        if(inputNgsildPath) {
          document.querySelectorAll('.FishProcessing_Page_ngsildPath').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildPath.getAttribute('value');
            else
              item.textContent = inputNgsildPath.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_ngsildPath'));
        }

        if(inputNgsildContext) {
          document.querySelectorAll('.FishProcessing_Page_ngsildContext').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildContext.getAttribute('value');
            else
              item.textContent = inputNgsildContext.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_ngsildContext'));
        }

        if(inputNgsildData) {
          document.querySelectorAll('.FishProcessing_Page_ngsildData').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildData.getAttribute('value');
            else
              item.textContent = inputNgsildData.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_ngsildData'));
        }

        if(inputAddress) {
          document.querySelectorAll('.FishProcessing_Page_address').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAddress.getAttribute('value');
            else
              item.textContent = inputAddress.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_address'));
        }

        if(inputClassCanonicalName) {
          document.querySelectorAll('.FishProcessing_Page_classCanonicalName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalName.getAttribute('value');
            else
              item.textContent = inputClassCanonicalName.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_classCanonicalName'));
        }

        if(inputClassSimpleName) {
          document.querySelectorAll('.FishProcessing_Page_classSimpleName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassSimpleName.getAttribute('value');
            else
              item.textContent = inputClassSimpleName.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_classSimpleName'));
        }

        if(inputClassCanonicalNames) {
          document.querySelectorAll('.FishProcessing_Page_classCanonicalNames').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalNames.getAttribute('value');
            else
              item.textContent = inputClassCanonicalNames.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_classCanonicalNames'));
        }

        if(inputSessionId) {
          document.querySelectorAll('.FishProcessing_Page_sessionId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSessionId.getAttribute('value');
            else
              item.textContent = inputSessionId.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_sessionId'));
        }

        if(inputUserKey) {
          document.querySelectorAll('.FishProcessing_Page_userKey').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserKey.getAttribute('value');
            else
              item.textContent = inputUserKey.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_userKey'));
        }

        if(inputSaves) {
          document.querySelectorAll('.FishProcessing_Page_saves').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSaves.getAttribute('value');
            else
              item.textContent = inputSaves.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_saves'));
        }

        if(inputObjectTitle) {
          document.querySelectorAll('.FishProcessing_Page_objectTitle').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectTitle.getAttribute('value');
            else
              item.textContent = inputObjectTitle.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_objectTitle'));
        }

        if(inputDisplayPage) {
          document.querySelectorAll('.FishProcessing_Page_displayPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDisplayPage.getAttribute('value');
            else
              item.textContent = inputDisplayPage.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_displayPage'));
        }

        if(inputDisplayPageFrFR) {
          document.querySelectorAll('.FishProcessing_Page_displayPageFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDisplayPageFrFR.getAttribute('value');
            else
              item.textContent = inputDisplayPageFrFR.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_displayPageFrFR'));
        }

        if(inputEditPage) {
          document.querySelectorAll('.FishProcessing_Page_editPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEditPage.getAttribute('value');
            else
              item.textContent = inputEditPage.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_editPage'));
        }

        if(inputEditPageFrFR) {
          document.querySelectorAll('.FishProcessing_Page_editPageFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEditPageFrFR.getAttribute('value');
            else
              item.textContent = inputEditPageFrFR.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_editPageFrFR'));
        }

        if(inputUserPage) {
          document.querySelectorAll('.FishProcessing_Page_userPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserPage.getAttribute('value');
            else
              item.textContent = inputUserPage.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_userPage'));
        }

        if(inputUserPageFrFR) {
          document.querySelectorAll('.FishProcessing_Page_userPageFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserPageFrFR.getAttribute('value');
            else
              item.textContent = inputUserPageFrFR.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_userPageFrFR'));
        }

        if(inputDownload) {
          document.querySelectorAll('.FishProcessing_Page_download').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDownload.getAttribute('value');
            else
              item.textContent = inputDownload.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_download'));
        }

        if(inputDownloadFrFR) {
          document.querySelectorAll('.FishProcessing_Page_downloadFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDownloadFrFR.getAttribute('value');
            else
              item.textContent = inputDownloadFrFR.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_downloadFrFR'));
        }

        if(inputObjectSuggest) {
          document.querySelectorAll('.FishProcessing_Page_objectSuggest').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectSuggest.getAttribute('value');
            else
              item.textContent = inputObjectSuggest.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_objectSuggest'));
        }

        if(inputObjectText) {
          document.querySelectorAll('.FishProcessing_Page_objectText').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectText.getAttribute('value');
            else
              item.textContent = inputObjectText.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_objectText'));
        }

        if(inputSolrId) {
          document.querySelectorAll('.FishProcessing_Page_solrId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSolrId.getAttribute('value');
            else
              item.textContent = inputSolrId.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_solrId'));
        }

        if(inputEntityShortId) {
          document.querySelectorAll('.FishProcessing_Page_entityShortId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEntityShortId.getAttribute('value');
            else
              item.textContent = inputEntityShortId.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_entityShortId'));
        }

        if(inputAreaServedColors) {
          document.querySelectorAll('.FishProcessing_Page_areaServedColors').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAreaServedColors.getAttribute('value');
            else
              item.textContent = inputAreaServedColors.textContent;
          });
          addGlow(document.querySelector('.FishProcessing_Page_areaServedColors'));
        }
...
function pageGraphFishProcessing(apiRequest) {
  ...
  if(r) {
    ...
    if(json['facetCounts']) {
      ...
      if(facetCounts['facetPivot'] && facetCounts['facetRanges']) {
        ...
        var layout = {};
        if(range) {
          layout['title'] = 'usine de transformation du poissons';
          layout['xaxis'] = {
          ...
          Plotly.react('htmBodyGraphFishProcessingPage', data, layout);
        }
      }
    }
    ...
    window.mapLayers = {};
    window.bounds = null;
    if(listFishProcessing.filter(o => o.location)) {
      window.bounds = L.latLngBounds(listFishProcessing.filter(o => o.location).map((c) => {
        return [c.location.coordinates[1], c.location.coordinates[0]];
      }));
    }
    function onEachFeature(feature, layer) {
      let popupContent = htmTooltipFishProcessing(feature, layer);
      layer.bindPopup(popupContent);
      window.mapLayers[feature.properties.id] = layer;
    };
    if(window.mapFishProcessing) {
      window.geoJSONFishProcessing.clearLayers();
        ...
        if(result.location) {
          ...
          shapes.forEach(function(shape, index) {
            ...
            var layerGeoJson = L.geoJSON(features, {
              ...
              , style: jsStyleFishProcessing
              , pointToLayer: function(feature, latlng) {
                return L.circleMarker(latlng, jsStyleFishProcessing(feature));
              }
            });
            window.geoJSONFishProcessing.addLayer(layerGeoJson);
            ...
            window.geoJSONFishProcessing.addLayer(layerSvg);
          });
        }
    ...
    } else if(document.getElementById('htmBodyGraphLocationFishProcessingPage')) {
      window.mapFishProcessing = L.map('htmBodyGraphLocationFishProcessingPage', {
        position: 'topright'
        , zoomControl: true
      ...
      window.mapFishProcessing.zoomControl.setPosition('topright');
      var data = [];
      var layout = {};
      ...
      legend.onAdd = jsLegendFishProcessing;
      L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
        maxZoom: 19,
        attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'
      }).addTo(window.mapFishProcessing);

      if(window.bounds && window['DEFAULT_MAP_ZOOM'] && window.bounds.getNorthEast()) {
        if(listFishProcessing.length == 1) {
          window.mapFishProcessing.setView(window.bounds.getNorthEast(), window['DEFAULT_MAP_ZOOM']);
        } else {
          window.mapFishProcessing.fitBounds(window.bounds);
        }
      } else {
        if(window['DEFAULT_MAP_LOCATION'] && window['DEFAULT_MAP_ZOOM'])
          window.mapFishProcessing.setView([window['DEFAULT_MAP_LOCATION']['coordinates'][1], window['DEFAULT_MAP_LOCATION']['coordinates'][0]], window['DEFAULT_MAP_ZOOM']);
        else if(window['DEFAULT_MAP_ZOOM'])
          window.mapFishProcessing.setView(null, window['DEFAULT_MAP_ZOOM']);
        else if(window['DEFAULT_MAP_LOCATION'])
          window.mapFishProcessing.setView([window['DEFAULT_MAP_LOCATION']['coordinates'][1], window['DEFAULT_MAP_LOCATION']['coordinates'][0]]);
      }

      layout['margin'] = { r: 0, t: 0, b: 0, l: 0 };
      window.geoJSONFishProcessing = L.geoJSON().addTo(window.mapFishProcessing);
      window.listFishProcessing.forEach((result, index) => {
        if(result.location) {
          var shapes = [];
          if(Array.isArray(result.location))
      ...
      window.mapFishProcessing.on('popupopen', function(e) {
        if(e.popup._source) {
          var feature = e.popup._source.feature;
          jsTooltipFishProcessing(e, feature);
        }
      });
      const drawnItems = new L.FeatureGroup();
      window.mapFishProcessing.addLayer(drawnItems);
      ...
      window.mapFishProcessing.addControl(drawControl);
      window.mapFishProcessing.on(L.Draw.Event.CREATED, function (event) {
        ...
        if(event.layerType == 'marker') {
          contextmenuItems.push({
            ...
            , callback: function(event2) {
              patchFishProcessingLocation(event.layer, { coordinates: [event.layer.getLatLng()['lng'], event.layer.getLatLng()['lat']], type: "Point" });
            }
          });
        }
        if(event.layerType == 'polygon') {
          contextmenuItems.push({
            ...
            , callback: function(event2) {
              ...
              patchFishProcessingArea(event.layer, { coordinates: latLngs, type: "Polygon" });
            }
          });
        }
        if(event.layerType == 'polyline') {
          contextmenuItems.push({
            ...
            , callback: function(event2) {
              ...
              patchFishProcessingArea(event.layer, { coordinates: latLngs, type: "LineString" });
            }
          });
        }
...
function patchFishProcessingLocation(target, location) {
  patchFishProcessingVal([{ name: 'softCommit', value: 'true' }, { name: 'fq', value: 'entityShortId:' + result.entityShortId }]
...
function patchFishProcessingArea(target, areaServed) {
  patchFishProcessingVal([{ name: 'softCommit', value: 'true' }, { name: 'fq', value: 'entityShortId:' + result.entityShortId }]
...
function animateStats() {
  document.querySelector('#pageSearchVal-fqFishProcessing_time').innerText = '';
  searchPage('FishProcessing', function() {
    ...
    let animateInterval = window.setInterval(() => {
      ...
      document.querySelector('#fqFishProcessing_time').value = x;
      document.querySelector('#fqFishProcessing_time').onchange();
      searchPage('FishProcessing');
    }, speedRate);
  });
}
...
async function searchFishProcessing($formFilters, success, error) {
  var filters = searchFishProcessingFilters($formFilters);
  if(success == null)
    success = function( data, textStatus, jQxhr ) {};
  if(error == null)
    error = function( jqXhr, target2 ) {};

  searchFishProcessingVals(filters, target, success, error);
}

function searchFishProcessingFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function searchFishProcessingVals(filters, target, success, error) {


  fetch(
    '/en-us/api/fish-processing?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
...
function suggestFishProcessingObjectSuggest($formFilters, $list, target) {
  success = function( data, textStatus, jQxhr ) {
    if($list) {
      $list.innerHTML = '';
      data['list'].forEach((o, i) => {
        var $i = document.querySelector('<i class="fa-duotone fa-regular fa-industry-windows"></i>');
  ...
  error = function( jqXhr, target2 ) {};
  searchFishProcessingVals($formFilters, target, success, error);
}

// GET //

async function getFishProcessing(pk) {
  fetch(
...
async function patchFishProcessing($formFilters, $formValues, target, entityShortId, success, error) {
  var filters = patchFishProcessingFilters($formFilters);
  ...
  patchFishProcessingVals(entityShortId == null ? deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:'entityShortId:' + entityShortId}], vals, target, success, error);
}

function patchFishProcessingFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function patchFishProcessingVal(filters, v, val, target, success, error) {
  var vals = {};
  vals[v] = val;
  patchFishProcessingVals(filters, vals, target, success, error);
}

function patchFishProcessingVals(filters, vals, target, success, error) {
  fetch(
    '/en-us/api/fish-processing?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function postFishProcessing($formValues, target, success, error) {
  var vals = {};
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
...
function postFishProcessingVals(vals, target, success, error) {
  fetch(
    '/en-us/api/fish-processing'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function deleteFishProcessing(target, entityShortId, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPageFrFR'];
  ...
  fetch(
    '/en-us/api/fish-processing/' + encodeURIComponent(entityShortId)
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportFishProcessing($formValues, target, entityShortId, success, error) {
  var json = $formValues.querySelector('.PUTImport_searchList')?.value;
  if(json != null && json !== '')
    putimportFishProcessingVals(JSON.parse(json), target, success, error);
}

function putimportFishProcessingVals(json, target, success, error) {
  fetch(
...
async function deletefilterFishProcessing(target, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPageFrFR'];
