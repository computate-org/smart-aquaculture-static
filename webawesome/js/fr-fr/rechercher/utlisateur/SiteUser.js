...
async function websocketSiteUser(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketSiteUser', function (error, message) {
      ...
      $headerSpan.innerText = 'modify utilisateur du sites in ' + json.timeRemaining;
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        if(inputDisplayPageFrFR) {
          document.querySelectorAll('.SiteUser_Page_displayPageFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDisplayPageFrFR.getAttribute('value');
            else
              item.textContent = inputDisplayPageFrFR.textContent;
          });
          addGlow(document.querySelector('.SiteUser_Page_displayPageFrFR'));
        }
        ...
        if(inputEditPageFrFR) {
          document.querySelectorAll('.SiteUser_Page_editPageFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEditPageFrFR.getAttribute('value');
            else
              item.textContent = inputEditPageFrFR.textContent;
          });
          addGlow(document.querySelector('.SiteUser_Page_editPageFrFR'));
        }
        ...
        if(inputUserPageFrFR) {
          document.querySelectorAll('.SiteUser_Page_userPageFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserPageFrFR.getAttribute('value');
            else
              item.textContent = inputUserPageFrFR.textContent;
          });
          addGlow(document.querySelector('.SiteUser_Page_userPageFrFR'));
        }
        ...
        if(inputDownloadFrFR) {
          document.querySelectorAll('.SiteUser_Page_downloadFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDownloadFrFR.getAttribute('value');
            else
              item.textContent = inputDownloadFrFR.textContent;
          });
          addGlow(document.querySelector('.SiteUser_Page_downloadFrFR'));
        }
  ...
  if(r) {
    ...
    if(json['facetCounts']) {
      ...
      if(facetCounts['facetPivot'] && facetCounts['facetRanges']) {
        ...
        var layout = {};
        if(range) {
          layout['title'] = 'utilisateur du sites';
          layout['xaxis'] = {
