...
async function websocketFeed(success) {
  window.eventBus.onopen = function () {

    window.eventBus.registerHandler('websocketFeed', function (error, message) {
      ...
      $headerSpan.innerText = 'modify nourritures in ' + json.timeRemaining;
...
async function websocketFeedInner(apiRequest) {
  ...
  if(entityShortId != null && vars.length > 0) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        window.listFeed = JSON.parse($response.querySelector('.pageForm .listFeed')?.value);
...
function pageGraphFeed(apiRequest) {
  ...
  if(r) {
    ...
    if(json['facetCounts']) {
      ...
      if(facetCounts['facetPivot'] && facetCounts['facetRanges']) {
        ...
        if(range) {
          ...
          Plotly.react('htmBodyGraphFeedPage', data, layout);
        }
      }
    }
    ...
    window.mapLayers = {};
    window.bounds = null;
    if(listFeed.filter(o => o.location)) {
      window.bounds = L.latLngBounds(listFeed.filter(o => o.location).map((c) => {
        return [c.location.coordinates[1], c.location.coordinates[0]];
      }));
    }
    function onEachFeature(feature, layer) {
      let popupContent = htmTooltipFeed(feature, layer);
      layer.bindPopup(popupContent);
      window.mapLayers[feature.properties.id] = layer;
    };
    if(window.mapFeed) {
      window.geoJSONFeed.clearLayers();
          ...
          shapes.forEach(function(shape, index) {
            ...
            var layerGeoJson = L.geoJSON(features, {
              ...
              , pointToLayer: function(feature, latlng) {
                return L.circleMarker(latlng, jsStyleFeed(feature));
              }
            });
            window.geoJSONFeed.addLayer(layerGeoJson);
          });
        }
    ...
    } else if(document.getElementById('htmBodyGraphLocationFeedPage')) {
      window.mapFeed = L.map('htmBodyGraphLocationFeedPage', {
        position: 'topright'
        , zoomControl: true
      ...
      window.mapFeed.zoomControl.setPosition('topright');
      var data = [];
      var layout = {};
      ...
      legend.onAdd = jsLegendFeed;
      L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
        maxZoom: 19,
        attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'
      }).addTo(window.mapFeed);

      if(window.bounds && window['DEFAULT_MAP_ZOOM'] && window.bounds.getNorthEast()) {
        if(listFeed.length == 1) {
          window.mapFeed.setView(window.bounds.getNorthEast(), window['DEFAULT_MAP_ZOOM']);
        } else {
          window.mapFeed.fitBounds(window.bounds);
        }
      } else {
        if(window['DEFAULT_MAP_LOCATION'] && window['DEFAULT_MAP_ZOOM'])
          window.mapFeed.setView([window['DEFAULT_MAP_LOCATION']['coordinates'][1], window['DEFAULT_MAP_LOCATION']['coordinates'][0]], window['DEFAULT_MAP_ZOOM']);
        else if(window['DEFAULT_MAP_ZOOM'])
          window.mapFeed.setView(null, window['DEFAULT_MAP_ZOOM']);
        else if(window['DEFAULT_MAP_LOCATION'])
          window.mapFeed.setView([window['DEFAULT_MAP_LOCATION']['coordinates'][1], window['DEFAULT_MAP_LOCATION']['coordinates'][0]]);
      }

      layout['margin'] = { r: 0, t: 0, b: 0, l: 0 };
      window.geoJSONFeed = L.geoJSON().addTo(window.mapFeed);
      window.listFeed.forEach((result, index) => {
        if(result.location) {
          var shapes = [];
          if(Array.isArray(result.location))
      ...
      window.mapFeed.on('popupopen', function(e) {
        if(e.popup._source) {
          var feature = e.popup._source.feature;
          jsTooltipFeed(e, feature);
        }
      });
      const drawnItems = new L.FeatureGroup();
      window.mapFeed.addLayer(drawnItems);
      ...
      window.mapFeed.addControl(drawControl);
      window.mapFeed.on(L.Draw.Event.CREATED, function (event) {
...
function animateStats() {
  document.querySelector('#pageSearchVal-fqFeed_time').innerText = '';
  searchPage('Feed', function() {
    ...
    let animateInterval = window.setInterval(() => {
      ...
      document.querySelector('#fqFeed_time').value = x;
      document.querySelector('#fqFeed_time').onchange();
      searchPage('Feed');
    }, speedRate);
  });
}
...
async function searchFeed($formFilters, success, error) {
  var filters = searchFeedFilters($formFilters);
  if(success == null)
    success = function( data, textStatus, jQxhr ) {};
  if(error == null)
    error = function( jqXhr, target2 ) {};

  searchFeedVals(filters, target, success, error);
}

function searchFeedFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function searchFeedVals(filters, target, success, error) {


  fetch(
    '/en-us/api/feed?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
...
function suggestFeedObjectSuggest($formFilters, $list, target) {
  success = function( data, textStatus, jQxhr ) {
    if($list) {
      $list.innerHTML = '';
  ...
  error = function( jqXhr, target2 ) {};
  searchFeedVals($formFilters, target, success, error);
}

// GET //

async function getFeed(pk) {
  fetch(
...
async function patchFeed($formFilters, $formValues, target, entityShortId, success, error) {
  var filters = patchFeedFilters($formFilters);
...
function patchFeedFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function patchFeedVal(filters, v, val, target, success, error) {
  var vals = {};
  vals[v] = val;
  patchFeedVals(filters, vals, target, success, error);
}

function patchFeedVals(filters, vals, target, success, error) {
  fetch(
    '/en-us/api/feed?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function postFeed($formValues, target, success, error) {
  var vals = {};
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
...
function postFeedVals(vals, target, success, error) {
  fetch(
    '/en-us/api/feed'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function deleteFeed(target, entityShortId, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPageFrFR'];
  ...
  fetch(
    '/en-us/api/feed/' + encodeURIComponent(entityShortId)
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportFeed($formValues, target, entityShortId, success, error) {
  var json = $formValues.querySelector('.PUTImport_searchList')?.value;
  if(json != null && json !== '')
    putimportFeedVals(JSON.parse(json), target, success, error);
}

function putimportFeedVals(json, target, success, error) {
  fetch(
...
async function deletefilterFeed(target, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPageFrFR'];
