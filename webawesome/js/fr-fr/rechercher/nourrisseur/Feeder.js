...
async function websocketFeeder(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketFeeder', function (error, message) {
      ...
      $headerSpan.innerText = 'modify nourrisseurs in ' + json.timeRemaining;
...
async function websocketFeederInner(apiRequest) {
  ...
  if(entityShortId != null && vars.length > 0) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        if(vars.includes('pk'))
          inputPk = $response.querySelector('.Feeder_Page_pk');
        if(vars.includes('created'))
          inputCreated = $response.querySelector('.Feeder_Page_created');
        if(vars.includes('modified'))
          inputModified = $response.querySelector('.Feeder_Page_modified');
        if(vars.includes('archived'))
          inputArchived = $response.querySelector('.Feeder_Page_archived');
        if(vars.includes('name'))
          inputName = $response.querySelector('.Feeder_Page_name');
        if(vars.includes('description'))
          inputDescription = $response.querySelector('.Feeder_Page_description');
        if(vars.includes('location'))
          inputLocation = $response.querySelector('.Feeder_Page_location');
        if(vars.includes('areaServed'))
          inputAreaServed = $response.querySelector('.Feeder_Page_areaServed');
        if(vars.includes('id'))
          inputId = $response.querySelector('.Feeder_Page_id');
        if(vars.includes('ngsildTenant'))
          inputNgsildTenant = $response.querySelector('.Feeder_Page_ngsildTenant');
        if(vars.includes('ngsildPath'))
          inputNgsildPath = $response.querySelector('.Feeder_Page_ngsildPath');
        if(vars.includes('ngsildContext'))
          inputNgsildContext = $response.querySelector('.Feeder_Page_ngsildContext');
        if(vars.includes('ngsildData'))
          inputNgsildData = $response.querySelector('.Feeder_Page_ngsildData');
        if(vars.includes('address'))
          inputAddress = $response.querySelector('.Feeder_Page_address');
        if(vars.includes('alternateName'))
          inputAlternateName = $response.querySelector('.Feeder_Page_alternateName');
        if(vars.includes('dataProvider'))
          inputDataProvider = $response.querySelector('.Feeder_Page_dataProvider');
        if(vars.includes('dateCreated'))
          inputDateCreated = $response.querySelector('.Feeder_Page_dateCreated');
        if(vars.includes('dateModified'))
          inputDateModified = $response.querySelector('.Feeder_Page_dateModified');
        if(vars.includes('owner'))
          inputOwner = $response.querySelector('.Feeder_Page_owner');
        if(vars.includes('relatedSource'))
          inputRelatedSource = $response.querySelector('.Feeder_Page_relatedSource');
        if(vars.includes('seeAlso'))
          inputSeeAlso = $response.querySelector('.Feeder_Page_seeAlso');
        if(vars.includes('source'))
          inputSource = $response.querySelector('.Feeder_Page_source');
        if(vars.includes('classCanonicalName'))
          inputClassCanonicalName = $response.querySelector('.Feeder_Page_classCanonicalName');
        if(vars.includes('classSimpleName'))
          inputClassSimpleName = $response.querySelector('.Feeder_Page_classSimpleName');
        if(vars.includes('classCanonicalNames'))
          inputClassCanonicalNames = $response.querySelector('.Feeder_Page_classCanonicalNames');
        if(vars.includes('sessionId'))
          inputSessionId = $response.querySelector('.Feeder_Page_sessionId');
        if(vars.includes('userKey'))
          inputUserKey = $response.querySelector('.Feeder_Page_userKey');
        if(vars.includes('saves'))
          inputSaves = $response.querySelector('.Feeder_Page_saves');
        if(vars.includes('objectTitle'))
          inputObjectTitle = $response.querySelector('.Feeder_Page_objectTitle');
        if(vars.includes('displayPage'))
          inputDisplayPage = $response.querySelector('.Feeder_Page_displayPage');
        if(vars.includes('displayPageFrFR'))
          inputDisplayPageFrFR = $response.querySelector('.Feeder_Page_displayPageFrFR');
        if(vars.includes('editPage'))
          inputEditPage = $response.querySelector('.Feeder_Page_editPage');
        if(vars.includes('editPageFrFR'))
          inputEditPageFrFR = $response.querySelector('.Feeder_Page_editPageFrFR');
        if(vars.includes('userPage'))
          inputUserPage = $response.querySelector('.Feeder_Page_userPage');
        if(vars.includes('userPageFrFR'))
          inputUserPageFrFR = $response.querySelector('.Feeder_Page_userPageFrFR');
        if(vars.includes('download'))
          inputDownload = $response.querySelector('.Feeder_Page_download');
        if(vars.includes('downloadFrFR'))
          inputDownloadFrFR = $response.querySelector('.Feeder_Page_downloadFrFR');
        if(vars.includes('objectSuggest'))
          inputObjectSuggest = $response.querySelector('.Feeder_Page_objectSuggest');
        if(vars.includes('objectText'))
          inputObjectText = $response.querySelector('.Feeder_Page_objectText');
        if(vars.includes('solrId'))
          inputSolrId = $response.querySelector('.Feeder_Page_solrId');
        if(vars.includes('areaServedColors'))
          inputAreaServedColors = $response.querySelector('.Feeder_Page_areaServedColors');
        if(vars.includes('areaServedTitles'))
          inputAreaServedTitles = $response.querySelector('.Feeder_Page_areaServedTitles');
        if(vars.includes('areaServedLinks'))
          inputAreaServedLinks = $response.querySelector('.Feeder_Page_areaServedLinks');
        if(vars.includes('entityShortId'))
          inputEntityShortId = $response.querySelector('.Feeder_Page_entityShortId');

        jsWebsocketFeeder(entityShortId, vars, $response);
        window.result = JSON.parse($response.querySelector('.pageForm .result')?.value);
        window.listFeeder = JSON.parse($response.querySelector('.pageForm .listFeeder')?.value);


        if(inputPk) {
          document.querySelectorAll('.Feeder_Page_pk').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputPk.getAttribute('value');
            else
              item.textContent = inputPk.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_pk'));
        }

        if(inputCreated) {
          document.querySelectorAll('.Feeder_Page_created').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputCreated.getAttribute('value');
            else
              item.textContent = inputCreated.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_created'));
        }

        if(inputModified) {
          document.querySelectorAll('.Feeder_Page_modified').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputModified.getAttribute('value');
            else
              item.textContent = inputModified.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_modified'));
        }

        if(inputArchived) {
          document.querySelectorAll('.Feeder_Page_archived').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputArchived.getAttribute('value');
            else
              item.textContent = inputArchived.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_archived'));
        }

        if(inputName) {
          document.querySelectorAll('.Feeder_Page_name').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputName.getAttribute('value');
            else
              item.textContent = inputName.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_name'));
        }

        if(inputDescription) {
          document.querySelectorAll('.Feeder_Page_description').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDescription.getAttribute('value');
            else
              item.textContent = inputDescription.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_description'));
        }

        if(inputLocation) {
          document.querySelectorAll('.Feeder_Page_location').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputLocation.getAttribute('value');
            else
              item.textContent = inputLocation.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_location'));
        }

        if(inputAreaServed) {
          document.querySelectorAll('.Feeder_Page_areaServed').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAreaServed.getAttribute('value');
            else
              item.textContent = inputAreaServed.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_areaServed'));
        }

        if(inputId) {
          document.querySelectorAll('.Feeder_Page_id').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputId.getAttribute('value');
            else
              item.textContent = inputId.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_id'));
        }

        if(inputNgsildTenant) {
          document.querySelectorAll('.Feeder_Page_ngsildTenant').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildTenant.getAttribute('value');
            else
              item.textContent = inputNgsildTenant.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_ngsildTenant'));
        }

        if(inputNgsildPath) {
          document.querySelectorAll('.Feeder_Page_ngsildPath').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildPath.getAttribute('value');
            else
              item.textContent = inputNgsildPath.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_ngsildPath'));
        }

        if(inputNgsildContext) {
          document.querySelectorAll('.Feeder_Page_ngsildContext').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildContext.getAttribute('value');
            else
              item.textContent = inputNgsildContext.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_ngsildContext'));
        }

        if(inputNgsildData) {
          document.querySelectorAll('.Feeder_Page_ngsildData').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputNgsildData.getAttribute('value');
            else
              item.textContent = inputNgsildData.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_ngsildData'));
        }

        if(inputAddress) {
          document.querySelectorAll('.Feeder_Page_address').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAddress.getAttribute('value');
            else
              item.textContent = inputAddress.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_address'));
        }

        if(inputAlternateName) {
          document.querySelectorAll('.Feeder_Page_alternateName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAlternateName.getAttribute('value');
            else
              item.textContent = inputAlternateName.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_alternateName'));
        }

        if(inputDataProvider) {
          document.querySelectorAll('.Feeder_Page_dataProvider').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDataProvider.getAttribute('value');
            else
              item.textContent = inputDataProvider.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_dataProvider'));
        }

        if(inputDateCreated) {
          document.querySelectorAll('.Feeder_Page_dateCreated').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDateCreated.getAttribute('value');
            else
              item.textContent = inputDateCreated.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_dateCreated'));
        }

        if(inputDateModified) {
          document.querySelectorAll('.Feeder_Page_dateModified').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDateModified.getAttribute('value');
            else
              item.textContent = inputDateModified.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_dateModified'));
        }

        if(inputOwner) {
          document.querySelectorAll('.Feeder_Page_owner').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputOwner.getAttribute('value');
            else
              item.textContent = inputOwner.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_owner'));
        }

        if(inputRelatedSource) {
          document.querySelectorAll('.Feeder_Page_relatedSource').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputRelatedSource.getAttribute('value');
            else
              item.textContent = inputRelatedSource.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_relatedSource'));
        }

        if(inputSeeAlso) {
          document.querySelectorAll('.Feeder_Page_seeAlso').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSeeAlso.getAttribute('value');
            else
              item.textContent = inputSeeAlso.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_seeAlso'));
        }

        if(inputSource) {
          document.querySelectorAll('.Feeder_Page_source').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSource.getAttribute('value');
            else
              item.textContent = inputSource.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_source'));
        }

        if(inputClassCanonicalName) {
          document.querySelectorAll('.Feeder_Page_classCanonicalName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalName.getAttribute('value');
            else
              item.textContent = inputClassCanonicalName.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_classCanonicalName'));
        }

        if(inputClassSimpleName) {
          document.querySelectorAll('.Feeder_Page_classSimpleName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassSimpleName.getAttribute('value');
            else
              item.textContent = inputClassSimpleName.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_classSimpleName'));
        }

        if(inputClassCanonicalNames) {
          document.querySelectorAll('.Feeder_Page_classCanonicalNames').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputClassCanonicalNames.getAttribute('value');
            else
              item.textContent = inputClassCanonicalNames.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_classCanonicalNames'));
        }

        if(inputSessionId) {
          document.querySelectorAll('.Feeder_Page_sessionId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSessionId.getAttribute('value');
            else
              item.textContent = inputSessionId.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_sessionId'));
        }

        if(inputUserKey) {
          document.querySelectorAll('.Feeder_Page_userKey').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserKey.getAttribute('value');
            else
              item.textContent = inputUserKey.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_userKey'));
        }

        if(inputSaves) {
          document.querySelectorAll('.Feeder_Page_saves').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSaves.getAttribute('value');
            else
              item.textContent = inputSaves.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_saves'));
        }

        if(inputObjectTitle) {
          document.querySelectorAll('.Feeder_Page_objectTitle').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectTitle.getAttribute('value');
            else
              item.textContent = inputObjectTitle.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_objectTitle'));
        }

        if(inputDisplayPage) {
          document.querySelectorAll('.Feeder_Page_displayPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDisplayPage.getAttribute('value');
            else
              item.textContent = inputDisplayPage.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_displayPage'));
        }

        if(inputDisplayPageFrFR) {
          document.querySelectorAll('.Feeder_Page_displayPageFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDisplayPageFrFR.getAttribute('value');
            else
              item.textContent = inputDisplayPageFrFR.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_displayPageFrFR'));
        }

        if(inputEditPage) {
          document.querySelectorAll('.Feeder_Page_editPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEditPage.getAttribute('value');
            else
              item.textContent = inputEditPage.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_editPage'));
        }

        if(inputEditPageFrFR) {
          document.querySelectorAll('.Feeder_Page_editPageFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEditPageFrFR.getAttribute('value');
            else
              item.textContent = inputEditPageFrFR.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_editPageFrFR'));
        }

        if(inputUserPage) {
          document.querySelectorAll('.Feeder_Page_userPage').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserPage.getAttribute('value');
            else
              item.textContent = inputUserPage.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_userPage'));
        }

        if(inputUserPageFrFR) {
          document.querySelectorAll('.Feeder_Page_userPageFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputUserPageFrFR.getAttribute('value');
            else
              item.textContent = inputUserPageFrFR.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_userPageFrFR'));
        }

        if(inputDownload) {
          document.querySelectorAll('.Feeder_Page_download').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDownload.getAttribute('value');
            else
              item.textContent = inputDownload.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_download'));
        }

        if(inputDownloadFrFR) {
          document.querySelectorAll('.Feeder_Page_downloadFrFR').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputDownloadFrFR.getAttribute('value');
            else
              item.textContent = inputDownloadFrFR.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_downloadFrFR'));
        }

        if(inputObjectSuggest) {
          document.querySelectorAll('.Feeder_Page_objectSuggest').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectSuggest.getAttribute('value');
            else
              item.textContent = inputObjectSuggest.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_objectSuggest'));
        }

        if(inputObjectText) {
          document.querySelectorAll('.Feeder_Page_objectText').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputObjectText.getAttribute('value');
            else
              item.textContent = inputObjectText.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_objectText'));
        }

        if(inputSolrId) {
          document.querySelectorAll('.Feeder_Page_solrId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputSolrId.getAttribute('value');
            else
              item.textContent = inputSolrId.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_solrId'));
        }

        if(inputAreaServedColors) {
          document.querySelectorAll('.Feeder_Page_areaServedColors').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAreaServedColors.getAttribute('value');
            else
              item.textContent = inputAreaServedColors.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_areaServedColors'));
        }

        if(inputAreaServedTitles) {
          document.querySelectorAll('.Feeder_Page_areaServedTitles').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAreaServedTitles.getAttribute('value');
            else
              item.textContent = inputAreaServedTitles.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_areaServedTitles'));
        }

        if(inputAreaServedLinks) {
          document.querySelectorAll('.Feeder_Page_areaServedLinks').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputAreaServedLinks.getAttribute('value');
            else
              item.textContent = inputAreaServedLinks.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_areaServedLinks'));
        }

        if(inputEntityShortId) {
          document.querySelectorAll('.Feeder_Page_entityShortId').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputEntityShortId.getAttribute('value');
            else
              item.textContent = inputEntityShortId.textContent;
          });
          addGlow(document.querySelector('.Feeder_Page_entityShortId'));
        }
...
function pageGraphFeeder(apiRequest) {
  ...
  if(r) {
    ...
    if(json['facetCounts']) {
      ...
      if(facetCounts['facetPivot'] && facetCounts['facetRanges']) {
        ...
        if(range) {
          ...
          Plotly.react('htmBodyGraphFeederPage', data, layout);
        }
      }
    }
    ...
    window.mapLayers = {};
    window.bounds = null;
    if(listFeeder.filter(o => o.location)) {
      window.bounds = L.latLngBounds(listFeeder.filter(o => o.location).map((c) => {
        return [c.location.coordinates[1], c.location.coordinates[0]];
      }));
    }
    function onEachFeature(feature, layer) {
      let popupContent = htmTooltipFeeder(feature, layer);
      layer.bindPopup(popupContent);
      window.mapLayers[feature.properties.id] = layer;
    };
    if(window.mapFeeder) {
      window.geoJSONFeeder.clearLayers();
    ...
    } else if(document.getElementById('htmBodyGraphLocationFeederPage')) {
      ...
      window.mapFeeder.zoomControl.setPosition('topright');
      var data = [];
      var layout = {};
      ...
      if(window.bounds && window['DEFAULT_MAP_ZOOM'] && window.bounds.getNorthEast()) {
        if(listFeeder.length == 1) {
          ...
          window.mapFeeder.fitBounds(window.bounds);
        }
      } else {
        if(window['DEFAULT_MAP_LOCATION'] && window['DEFAULT_MAP_ZOOM'])
          window.mapFeeder.setView([window['DEFAULT_MAP_LOCATION']['coordinates'][1], window['DEFAULT_MAP_LOCATION']['coordinates'][0]], window['DEFAULT_MAP_ZOOM']);
        else if(window['DEFAULT_MAP_ZOOM'])
          window.mapFeeder.setView(null, window['DEFAULT_MAP_ZOOM']);
        else if(window['DEFAULT_MAP_LOCATION'])
          window.mapFeeder.setView([window['DEFAULT_MAP_LOCATION']['coordinates'][1], window['DEFAULT_MAP_LOCATION']['coordinates'][0]]);
      }

      layout['margin'] = { r: 0, t: 0, b: 0, l: 0 };
      window.geoJSONFeeder = L.geoJSON().addTo(window.mapFeeder);
      window.listFeeder.forEach((result, index) => {
        if(result.location) {
          var shapes = [];
          if(Array.isArray(result.location))
      ...
      window.mapFeeder.on('popupopen', function(e) {
        if(e.popup._source) {
          var feature = e.popup._source.feature;
          jsTooltipFeeder(e, feature);
        }
      });
      const drawnItems = new L.FeatureGroup();
      window.mapFeeder.addLayer(drawnItems);
      ...
      window.mapFeeder.addControl(drawControl);
      window.mapFeeder.on(L.Draw.Event.CREATED, function (event) {
        ...
        if(event.layerType == 'marker') {
          contextmenuItems.push({
            ...
            , callback: function(event2) {
              patchFeederLocation(event.layer, { coordinates: [event.layer.getLatLng()['lng'], event.layer.getLatLng()['lat']], type: "Point" });
            }
          });
        }
        if(event.layerType == 'polygon') {
          contextmenuItems.push({
            ...
            , callback: function(event2) {
              ...
              patchFeederArea(event.layer, { coordinates: latLngs, type: "Polygon" });
            }
          });
        }
        if(event.layerType == 'polyline') {
          contextmenuItems.push({
            ...
            , callback: function(event2) {
              ...
              patchFeederArea(event.layer, { coordinates: latLngs, type: "LineString" });
            }
          });
        }
...
function patchFeederLocation(target, location) {
  patchFeederVal([{ name: 'softCommit', value: 'true' }, { name: 'fq', value: 'entityShortId:' + result.entityShortId }]
...
function patchFeederArea(target, areaServed) {
  patchFeederVal([{ name: 'softCommit', value: 'true' }, { name: 'fq', value: 'entityShortId:' + result.entityShortId }]
...
function animateStats() {
  document.querySelector('#pageSearchVal-fqFeeder_time').innerText = '';
  searchPage('Feeder', function() {
    ...
    let animateInterval = window.setInterval(() => {
      ...
      document.querySelector('#fqFeeder_time').value = x;
      document.querySelector('#fqFeeder_time').onchange();
      searchPage('Feeder');
    }, speedRate);
  });
}
...
async function searchFeeder($formFilters, success, error) {
  var filters = searchFeederFilters($formFilters);
  if(success == null)
    success = function( data, textStatus, jQxhr ) {};
  if(error == null)
    error = function( jqXhr, target2 ) {};

  searchFeederVals(filters, target, success, error);
}

function searchFeederFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function searchFeederVals(filters, target, success, error) {


  fetch(
    '/en-us/api/feeder?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
...
function suggestFeederObjectSuggest($formFilters, $list, target) {
  success = function( data, textStatus, jQxhr ) {
    if($list) {
      $list.innerHTML = '';
  ...
  error = function( jqXhr, target2 ) {};
  searchFeederVals($formFilters, target, success, error);
}

// GET //

async function getFeeder(pk) {
  fetch(
...
async function patchFeeder($formFilters, $formValues, target, entityShortId, success, error) {
  var filters = patchFeederFilters($formFilters);
  ...
  patchFeederVals(entityShortId == null ? deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:'entityShortId:' + entityShortId}], vals, target, success, error);
}

function patchFeederFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function patchFeederVal(filters, v, val, target, success, error) {
  var vals = {};
  vals[v] = val;
  patchFeederVals(filters, vals, target, success, error);
}

function patchFeederVals(filters, vals, target, success, error) {
  fetch(
    '/en-us/api/feeder?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function postFeeder($formValues, target, success, error) {
  var vals = {};
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
...
function postFeederVals(vals, target, success, error) {
  fetch(
    '/en-us/api/feeder'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function deleteFeeder(target, entityShortId, success, error) {
  ...
  fetch(
    '/en-us/api/feeder/' + encodeURIComponent(entityShortId)
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportFeeder($formValues, target, entityShortId, success, error) {
  var json = $formValues.querySelector('.PUTImport_searchList')?.value;
  if(json != null && json !== '')
    putimportFeederVals(JSON.parse(json), target, success, error);
}

function putimportFeederVals(json, target, success, error) {
  fetch(
